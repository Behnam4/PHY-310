...
\begin{document}
  ...
  \begin{enumerate}
    ...
    \textcolor{hwColor}{
      $
        \mathcal{L}(r, \phi, \dot{r}, \dot{\phi})=KE-PE=\dfrac{1}{2} \mu \left(\dot{r}^2+r^2 \dot{\phi}^2\right)-PE \\ 
        \\
        \rule{15cm}{1pt}
        \\
        \\
        \dfrac{\partial \mathcal{L}}{\partial \phi}=\dfrac{d}{dt} \left(\mu r^2 \dot{\phi}\right) 
        \\
        \\
        \therefore ~~~ \dot{\phi}=\dfrac{l}{\mu r^2}
        \\
        \rule{15cm}{1pt}
        \\
        \\
        ...
        \\
        \\
        \therefore ~~~ \dfrac{l^2}{\mu r^3}+F(r)=\mu \dddot{r}
        \\
        \\
        \rule{15cm}{1pt}
        \\
        \\
        r \equiv \dfrac{1}{\omega}, ~~~~ \dfrac{d}{dt}=\dfrac{d\phi}{dt} \dfrac{d}{d \phi}=\dfrac{l}{\mu r^2} \dfrac{d}{d\phi} \\
        \\
        \\
        \Longrightarrow \dot{r}=\dfrac{l}{\mu r^2} \dfrac{d}{d \phi} \dfrac{1}{\omega}=\dfrac{l \omega^2}{\mu} \dfrac{d}{d \phi} \dfrac{1}{\omega}  \\
        \\
        \\
        \therefore ~~~~ \ddot{r}=-\left(\dfrac{l \omega}{\mu}\right)^2 \dfrac{d^2 \omega}{d \phi^2}
        \\
        \\
        \rule{15cm}{1pt}
        \\
        \\
        \mu \ddot{r}-\dfrac{l^2}{\mu r^3}-F(r)=0 \\
        \\
        -l^2 \omega^2 \omega^{''}-l^2 \omega^3+\mu k \omega^5=0 \\
        \\
        \therefore ~~~~ \omega^{''}+\omega-\dfrac{\mu k \omega^3}{l^2}=0 \\
        \\
        \\
        \omega=\dfrac{1}{r}=\dfrac{1}{a cos(\phi)} \Longrightarrow \begin{cases}
          \omega^'=\dfrac{sin(\phi)}{a cos^2(\phi)} \\
          \\
          \omega^{''}=\dfrac{1}{a cos(\phi)}+\dfrac{2 sin^2(\phi)}{a cos^3(\phi)}
        \end{cases} \\
        \\
        \\
        \omega^{''}+\omega-\dfrac{\mu k \omega^3}{l^2}=0 \\
        \\
        \dfrac{2sin^2(\phi)}{a ~ cos^3(\phi)}+\dfrac{1}{a cos(\phi)}+\dfrac{1}{a cos(\phi)}-\dfrac{\mu k}{l^2 a^3 cos^3(\phi)}=0
        \\
        \\
        2a^2 sin^2(\phi)+2a^2 cos^2(\phi)-\dfrac{\mu k}{l^2}=0 \\
        \\
        \\
        \therefore ~~~ a=\pm l^{-1} \sqrt{\dfrac{\mu k}{2}}
        \\
        \\
        \rule{15cm}{1pt}
        \\
        \\
        \begin{cases}
          r=\dfrac{1}{l} \sqrt{\dfrac{\mu k}{2}} cos(\phi) \\
          \\
          \omega=\dfrac{l}{cos(\phi)} \sqrt{\dfrac{2}{\mu k}} \\
          \\
          \omega^'=\dfrac{l sin(\phi)}{cos^2(\phi)} \sqrt{\dfrac{2}{\mu k}} \\
          \\
          \omega^{''}=\dfrac{2l sin^2(\phi)}{cos^3(\phi)} \sqrt{\dfrac{2}{\mu k}}+\dfrac{l}{cos(\phi)} \sqrt{\dfrac{2}{\mu k}}
        \end{cases}
        \\
        \\
        \rule{15cm}{1pt}
        ...
        \\
        \\
        \dfrac{2l sin^2(\phi)}{cos^3(\phi)} \sqrt{\dfrac{2}{\mu k}}+\dfrac{l}{cos(\phi)} \sqrt{\dfrac{2}{\mu k}}+\dfrac{l}{cos(\phi)} \sqrt{\dfrac{2}{\mu k}}-\dfrac{\mu k}{l^2} \left(\dfrac{l}{cos(\phi)} \sqrt{\dfrac{2}{\mu k}}\right)^3=0 \\
        \\
        \\
        2l sin^2(\phi)+2lcos^2(\phi)-\dfrac{2 \mu k l}{\mu k}=0 \\
        \\
        \\
        2l-2l=0
      $
      \\
      \\
      Therefore, the solution $r=a cos(\phi)$ is a solution to the central force. 
    }
